...
#' @export
dglm_predict.continuous_deepglm <- function(data, dglm_model, type = NULL) {
...
#' @export
predict.deepglm <- function(object, ...) { #newdata, type = "factor") {
  args <- list(...)
...
#' @importFrom stats model.frame model.matrix predict
#' @export
dglm_predict.categorical_deepglm <- 
  function(data, dglm_model, type = "factor") {
...
#' @export
dglm_predict.survival_deepglm <- function(data, dglm_model, type = "expected") {

  mm <- model.matrix(dglm_model$formula, 
                     model.frame(dglm_model$formula, data))
  ret <- predict(dglm_model$model, mm[,-1, drop = FALSE])
  switch(type,
         "expected" = ret,
         stop(red("Unknown survival prediction type.")))
}
